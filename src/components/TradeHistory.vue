...
<div>2024.12.6 转出 1000 sui 到钱包做 T <span @click="jump('5e4R8XLG9noGZ28ez1LpaYcSZ2yM7YTRrfGzrcxhx8ZD')"
    class="digest">5e4R8XLG9noGZ28ez1LpaYcSZ2yM7YTRrfGzrcxhx8ZD</span> </div>
<div>{{ `sui 收益: ${suiGain.toFixed(9)}` }}</div>
<div>{{ `usdc 收益: ${usdcGain.toFixed(6)}` }}</div>
<div v-if="suiGain < 0">{{ `卖出 SUI 均价 ${- usdcGain / suiGain}` }}</div>
<div v-for="(tran, index) of suiTradeData" :key="index" class="transaction"
  ...
  <span class="detail">{{ tran.direction === direction.SELL ? `${tran.sui} SUI => ${tran.usdc} USDC` : `${tran.usdc}
    USDC => ${tran.sui} SUI` }}</span>
